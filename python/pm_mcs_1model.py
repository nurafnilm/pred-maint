...
# ========== KONFIGURASI LOGGING ========== #
LOG_FILE ="/home/edicoba/Documents/predictive_maintenance/pm_mcs_1model/pm_mcs_1model_log.log"
...
min_ae_vals = np.array([0.0, 36.0, 0.0, 0.0, 19.36, 0.0])
max_ae_vals = np.array([360.0, 100.0, 1332.0, 1.2, 44.25, 9.6])

# ========== KONFIGURASI csv ========== #
def write_csv_fallback(timestamp, feature, error, threshold):
    try:
        dt = datetime.datetime.fromisoformat(timestamp)
        filename = f"/home/edicoba/Documents/predictive_maintenance/pm_mcs_1model/fallback/pm_mcs_1model_fallback_{dt.strftime('%Y%m%d_%H')}.csv"
...
class SensorBuffer:
    ...
    def add_data(self, processed_sensor_data):
        with self.lock:
            ...
            logger.info(f"[Buffer] Masuk: angle: {processed_sensor_data.get('angle', '-')}, rainfall: {processed_sensor_data.get('rainfall', '-')}")
...
# Telegram Bot
TELEGRAM_TOKEN = "7906695159:AAE4lA0m1E6v5VoBcA2fVDeusw25mVYBX0k"
...
def write_telegram_metrics_csv(timestamp, num_messages, execution_time, status):
    try:
        dt = datetime.datetime.fromisoformat(timestamp)
        filename = f"/home/edicoba/Documents/predictive_maintenance/pm_mcs_1model/telegram_metrics/pm_mcs_1model_telegram_{dt.strftime('%Y%m%d_%H')}.csv"
        os.makedirs(os.path.dirname(filename), exist_ok=True)
...
def on_message(client, userdata, msg):
    try:
        logger.info(f"[MQTT] Pesan masuk topik {msg.topic}")
...
def write_interference_time_csv(timestamp, execution_time):
    try:
        dt = datetime.datetime.fromisoformat(timestamp)
        filename = f"/home/edicoba/Documents/predictive_maintenance/pm_mcs_1model/interference_time/pm_mcs_1model_interference_{dt.strftime('%Y%m%d_%H')}.csv"
        os.makedirs(os.path.dirname(filename), exist_ok=True)
...
# ========== THREAD PROSES LSTM AUTOENCODER DENGAN WAKTU INTERFERENSI ========== #
def process_autoencoder_input():
    thresholds = np.array([0.383023, 0.075581, 0.277977, 0.170791, 0.105172, 0.196772], dtype=np.float32)
    fitur_names = ["angle", "humidity", "pyrano", "rainfall", "temperature", "wind_speed"]
    ...
    while True:
        ...
        if len(data) == pengambilan_size:
            ...
            # Normalisasi input data untuk Autoencoder
            ...
            # Format input untuk model LSTM Autoencoder
            ...
            timestamp_now = datetime.datetime.now().isoformat()
            current_minute = datetime.datetime.now().minute  # Ambil menit saat ini
            anomalous_data_this_batch = False
            anomalies_per_batch = {}  # Untuk menyimpan error per fitur dalam batch ini
        ...
        time.sleep(60)  # Setel interval tidur untuk memproses setiap menit
