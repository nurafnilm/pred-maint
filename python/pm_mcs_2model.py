...
# ========== KONFIGURASI LOGGING ========== #
LOG_FILE = "/home/edicoba/Documents/predictive_maintenance/pm_mcs_2model/pm_mcs_2model_log.log"
...
min1_vals = np.array([0.0, 36.0, 0.0, 0.0, 19.36, 0.0, -1.0, -1.0])
max1_vals = np.array([360.0, 100.0, 1332.0, 1.2, 44.25, 9.6, 1.0, 1.0])

min2_vals = np.array([0.0, 36.0, 0.0, 0.0, 19.36, 0.0])
max2_vals = np.array([360.0, 100.0, 1332.0, 1.2, 44.25, 9.6])

# ========== KONFIGURASI SIMPAN CSV ========== #
def write_csv_fallback(timestamp, feature, error, threshold):
    try:
        dt = datetime.datetime.fromisoformat(timestamp)
        filename = f"/home/edicoba/Documents/predictive_maintenance/pm_mcs_2model/fallback/pm_mcs_2model_fallback_{dt.strftime('%Y%m%d_%H')}.csv"
...
class SensorBuffer:
    ...
    def add_data(self, processed_sensor_data):
        ...
        with self.lock:
            self.buffer.append(data_with_time)
            logger.info(f"[Buffer] Masuk: angle: {data_with_time.get('angle', '-')}, rainfall: {data_with_time.get('rainfall', '-')}, hour_sin: {str(hour_sin)[:5]}")
...
def periodic_memory_cleanup():
    while True:
        ...
        time.sleep(600)  # Pembersihan memori setiap 1 menit

# ========== KONFIGURASI MQTT DAN MODEL ========== #
MQTT_BROKER = "c-greenproject.org"
MQTT_PORT = 1883
MQTT_TOPIC = "testing_pm/climate"
MQTT_USERNAME = "eddystation"
MQTT_PASSWORD = "pwdMQTT@123"

MODEL_CNNLSTM_PATH = "/home/edicoba/Documents/predictive_maintenance/model/predicted_mcs_pls_32_64_batch1.tflite"
MODEL_AUTOENCODER_PATH = "/home/edicoba/Documents/predictive_maintenance/model/anomali_mcs_pls_64_batch1.tflite"
# ========== LOAD MODEL CNN-LSTM ========== #
...
# Telegram Bot
TELEGRAM_TOKEN = "7843742751:AAGsjaQ4S_AyN9CkOz3eB-kK1w-b3goFLcE"
...
def write_telegram_metrics_csv(timestamp, num_messages, execution_time, status):
    try:
        dt = datetime.datetime.fromisoformat(timestamp)
        filename = f"/home/edicoba/Documents/predictive_maintenance/pm_mcs_2model/telegram_metrics/pm_mcs_2model_telegram_{dt.strftime('%Y%m%d_%H')}.csv"
        os.makedirs(os.path.dirname(filename), exist_ok=True)
...
def on_message(client, userdata, msg):
    try:
        ...
        processed = {
            "angle": replace_null(data.get("angle")),
            "humidity": replace_null(data.get("humidity")),
            "pyrano": replace_null(data.get("pyrano")),
            "rainfall": replace_null(data.get("rainfall")),
            "temperature": replace_null(data.get("temperature")),
            "wind_speed": replace_null(data.get("wind_speed"))
        }
...
def write_interference_time_csv(timestamp, execution_time, model_name):
    try:
        dt = datetime.datetime.fromisoformat(timestamp)
        filename = f"/home/edicoba/Documents/predictive_maintenance/pm_mcs_2model/interference_time/pm_mcs_2model_interference_{dt.strftime('%Y%m%d_%H')}.csv"
        os.makedirs(os.path.dirname(filename), exist_ok=True)
    ...
    while True:
        ...
        if len(data) >= pengambilan_size:
            ...
            if len(chunk) == pengambilan_size:
                raw_data = np.array([[
                    item.get("angle", -9999),
                    item.get("humidity", -9999),
                    item.get("pyrano", -9999),
                    item.get("rainfall", -9999),
                    item.get("temperature", -9999),
                    item.get("wind_speed", -9999),
...
def process_autoencoder_input():
    thresholds = np.array([0.383023, 0.075581, 0.277977, 0.170791, 0.105172, 0.196772], dtype=np.float32)
    fitur_names = ["angle", "humidity", "pyrano", "rainfall",
    ...
    while True:
        if prediction_buffer.size() >= prediction_buffer.buffer.maxlen:
            ...
            denormalized_output = scaled_output * (max1_vals[:6] - min1_vals[:6]) + min1_vals[:6]
            ...
            timestamp_now = datetime.datetime.now().isoformat()
            current_minute = datetime.datetime.now().minute  # Menggunakan menit sekarang untuk pengiriman telegram
            anomalous_data_this_batch = False
            anomalies_per_batch = {}

            # Deteksi anomali berdasarkan nilai error
            ...
            # Kirim peringatan ke Telegram jika ada anomali dan satu menit telah berlalu
            if anomalous_data_this_batch and current_minute != last_telegram_sent_second:
                ...
                last_telegram_sent_second = current_minute  # Update pengiriman terakhir berdasarkan menit

        time.sleep(60)  # Proses setiap menit
