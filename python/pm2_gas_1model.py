...
# ========== KONFIGURASI LOGGING ========== #
LOG_FILE ="/home/edicoba2/Documents/predictive_maintenance/pm2_gas_1model/pm2_gas_1model_log.log"
...
def write_csv_fallback(timestamp, feature, error, threshold):
    try:
        ...
        filename = f"/home/edicoba2/Documents/predictive_maintenance/pm2_gas_1model/fallback/pm2_gas_1model_fallback_{dt.strftime('%Y%m%d_%H')}.csv"
...
def write_telegram_metrics_csv(timestamp, num_messages, execution_time, status):
    try:
        dt = datetime.datetime.fromisoformat(timestamp)
        filename = f"/home/edicoba2/Documents/predictive_maintenance/pm2_gas_1model/telegram_metrics/pm2_gas_1model_telegram_{dt.strftime('%Y%m%d_%H')}.csv"
        os.makedirs(os.path.dirname(filename), exist_ok=True)
...
def write_interference_time_csv(timestamp, execution_time):
    try:
        dt = datetime.datetime.fromisoformat(timestamp)
        filename = f"/home/edicoba2/Documents/predictive_maintenance/pm2_gas_1model/interference_time/pm2_gas_1model_interference_{dt.strftime('%Y%m%d_%H')}.csv"
        os.makedirs(os.path.dirname(filename), exist_ok=True)
