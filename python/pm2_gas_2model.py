...
# ========== KONFIGURASI LOGGING ========== #
LOG_FILE = "/home/edicoba2/Documents/predictive_maintenance/pm2_gas_2model/pm_gas_2model_log.log"
...
def write_csv_fallback(timestamp, feature, error, threshold):
    try:
        ...
        filename = f"/home/edicoba/Documents/predictive_maintenance/pm2_gas_2model/fallback/pm2_gas_2model_fallback_{dt.strftime('%Y%m%d_%H')}.csv"
...
# Telegram Bot
TELEGRAM_TOKEN = "8091702636:AAFAfjgQWryauWFvCX7KSghDCEaGTsJpYC4"
...
def write_telegram_metrics_csv(timestamp, num_messages, execution_time, status):
    try:
        dt = datetime.datetime.fromisoformat(timestamp)
        filename = f"/home/edicoba2/Documents/predictive_maintenance/pm2_gas_2model/telegram_metrics/pm_gas_2model_telegram_{dt.strftime('%Y%m%d_%H')}.csv"
        os.makedirs(os.path.dirname(filename), exist_ok=True)
...
def on_message(client, userdata, msg):
    try:
        now = datetime.datetime.now()
        # Proses hanya jika waktu saat ini berada dalam interval menit 0 sampai 30
        if 0 <= now.minute < 30:
            ...
            logger.info(f"[MQTT] Pesan masuk di luar rentang waktu (Menunggu waktu selanjutnya)")
    except Exception as e:
...
def write_interference_time_csv(timestamp, execution_time, model_name):
    try:
        dt = datetime.datetime.fromisoformat(timestamp)
        filename = f"/home/edicoba2/Documents/predictive_maintenance/pm2_gas_2model/interference_time/pm2_gas_2model_interference_{dt.strftime('%Y%m%d_%H')}.csv"
        os.makedirs(os.path.dirname(filename), exist_ok=True)
